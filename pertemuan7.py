...
Original file is located at
    https://colab.research.google.com/drive/1Bu8SPehu3ntyHRAhIDM4gYc_TrSO1DG1
"""

def matrix_multiplication(A, n):
    # Inisialisasi matriks C dengan nol
    C = [[0 for j in range(n)] for i in range(n)]

    # Perulangan untuk mengisi matriks C
    for i in range(n):
        for j in range(n):
            for k in range(n):
                C[i][j] += A[i][k]  # Menjumlahkan produk A[i][k] dengan nilai sebelumnya di C[i][j]

    return C

n = 3  # Ganti dengan ukuran matriks yang sesuai
A = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]  # Ganti dengan matriks A yang sesuai

result = matrix_multiplication(A, n)
